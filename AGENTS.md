...
- **고문** (윈도우 Cursor): 사서가 아니다. 헤르메스와 같이 `inbox/`만 쓴다. ingest·daily·감수·집 설계 금지. (2026-08-15)
...
| _(Hermes·고문 push)_ | 헤르메스 또는 고문이 inbox **push 성공**을 알리면 사서가 맥 볼트에서 `git pull` → 받은 파일·커밋을 **편집장에게 보고** → 필요 시 ingest |
...
헤르메스(VPS) 또는 **고문**(윈도우 Cursor)이 `inbox/`에 쓰고 **GitHub `osopung/Repo`에 push**하면:
...
2. **헤르메스·고문** — `inbox/`에 쓰면 바로 commit·push. 끝나면 사서에게 알린다.
