...
| 영역 | 내용 |
|------|------|
| **접속 시 ingest** | 편집장이 사서에게 **접속한 시점**에 `inbox/` 미처리 `.md`를 `library/notes/`로 재탄생 (핵심·사서 인사이트 초안 · 대기 중엔 쌓아 둠 · 핵심 없으면 보류 · **유튜브 링크 제외 — 지시 대기** 2026-09-09) |
...
| 초점 | 지금 관심·중요 일 **한두 가지** | 논지·Home·moc 손질 + **프로젝트 3칸** |
| 분량 | 제안 1~3줄 블록 | 좀 더 넓은 제안 |

**weekly 프로젝트 3칸** (2026-09-07 편집장 결정, [[library/notes/기회-3가지-작업시스템]] 「내 한 줄」): 진행 중 프로젝트마다 **이번 주 완료물 1 · 다음 우선순위 1 · 측정 지표 1**만 적는다. 뼈대는 ★5 [[library/projects/프로젝트-진행단계-템플릿]].
...
| 명령 | 동작 |
|------|------|
| _(접속 시)_ / `ingest <파일>` | 사서 **접속(대화 시작) 시점**에 inbox 미처리 ingest (**유튜브 링크 제외** — 지시 대기 2026-09-09). 특정 파일만 `ingest <파일>`. **내 한 줄**(초안) 필수. log append |
...
| `weekly` | Home·논지·moc 갱신 **제안** + **스킬 개선 후보 1개** ([[map/moc/스킬]] 상행 루프) + **프로젝트 3칸**(완료물 1 · 우선순위 1 · 측정 지표 1) — 사람 승인 후 반영 |

### Ingest 워크플로

**기본:** 사람이 파일마다 `ingest …`를 치지 않는다. inbox에 쌓아 두고, **사서에게 접속한 시점**에 소화한다. 단 **유튜브 링크·클립 메모는 자동 소화하지 않고** inbox에 두어 편집장 지시를 기다린다 (2026-09-09).

1. 세션 시작 시 `inbox/` 스캔 (README·숨김·유튜브 링크 제외) — **건수 제한 없이** 처리  
