...
| 대용량 스캔·영상·덤프 (갤러리 밖) | `오소풍-미디어-보관/` | 링크 또는 `media:` 필드 |
...
| Git (`osopung/Repo`) | 이력·복구 (Sync와 병행). inbox push는 **헤르메스**. **코덱스는 Git·inbox 경로가 아님** — 산출물은 편집장이 사서에게 전한다 |
